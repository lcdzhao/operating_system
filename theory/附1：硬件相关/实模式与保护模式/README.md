**历史**:
> - [Intel CPU 历史](https://github.com/lcdzhao/operating_system/tree/master/theory/%E9%99%841%EF%BC%9A%E7%A1%AC%E4%BB%B6%E7%9B%B8%E5%85%B3/CPU(Intel)%E5%8F%8A%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%8F%91%E5%B1%95%E5%8E%86%E5%8F%B2)
...
# 保护模式
为了让计算机可以进一步发展，让用户可以安全地下载并使用任何地方来的应用程序，这个时候对操作系统提出了更高的要求，即：需要保证多个应用之间的相互独立及安全性，且用户程序无法破环操作系统的数据(这样即使某个应用程序是恶意软件，也不会影响操作操作系统的代码(除非操作系统自己有漏洞))，这就要求**多个应用之间的数据都进行隔离，一个应用无法访问另一个应用的数据**。
...
- 分段寻址：通过`段选择子(cs、ds、ss、es等段寄存器) 与 段内偏移(ip、di、si、bp、sp等寄存器)`来找到当前应用的**线性(虚拟)内存地址**。

- 分页寻址：将**分段寻址**找到的**线性(虚拟)内存地址** 转化为实际的物理地址。(PS：**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此每个应用仅能访问到自己的数据。**。)

下面我们来详细分析一下这两个阶段。

## 分段寻址(虚拟内存)
### 目的
> 使用`段选择子(cs、ds、ss、es等段寄存器) 与 段内偏移(ip、di、si、bp、sp等寄存器)` 来对**当前应用程序的虚拟内存**进行读写访问。
### 虚拟内存
`虚拟内存`是实现"保护"的精髓，它使得**应用程序认为它拥有连续可用的内存（一个连续完整的地址空间，如32位操作系统下，应用认为自己拥有4GB的连续内存空间）**，而实际上物理内存通常被分隔成多个内存碎片(**如何将应用程序的虚拟内存映射为实际物理内存由操作系统和底层硬件控制，用户程序无法控制。通过这样使得用户程序仅能访问自己内存空间的内存，而无法取访问其它应用的内存空间**)，还有部分暂时存储在外部磁盘存储器的swap分区上，在需要时进行数据交换。

因此虚拟内存有两个非常重要的作用：

- **将应用程序自身内存管理与操作系统内存管理解耦，简化应用程序自身的内存管理**：应用程序认为自己拥有完整的地址空间，因此它只需要管理好自己的数据就行，不需要去感知操作系统或者其他应用的内存管理情况。

- 应用隔离：**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此每个应用仅能访问到自己的数据**。
...
> - 第0-1位(**RPL**): 表示**选择子的特权级**，共有四个特权级(0级，1级，2级，3级)。在程序中的每一个段都有一个特定的特权级，每当一个程序试图访问某一个段时，就将该程序所拥有的特权级(CS中的特权级)与要访问的特权级进行比较（数字越小级别越高），特权级高的才可以访问，否则不可以访问(见下一小节的CPL与DPL)。CPU只能访问同一个特权级或者级别较低特权级的段。
> 
...
![`TI = 0`时的寻址动画演示](README.assets/TI0.svg)
...
![`TI = 1`时的寻址动画演示](README.assets/TI1.svg)
...
> 
> `段描述符物理地址 = LDT开始地址(对应 LDT 段描述符中的段基址) + (Index * 段描述符大小))` —— **对应上图中的绿色的步骤1**
...
的方式来定位到虚拟内存的地址从而访问。(其中虚拟内存还需要进行进一步的转换才能变成物理内存，下一小节进行讲解)

#### x举例：给出逻辑地址：`0x21:0x12345678` 转换为线性(虚拟)地址
>
> STEP1: 选择子 `SEL = 0x21 = 0000000000100 0 01` 所代表的意思是：最后的`01`代表特权级`RPL=1`；`TI=0`代表选择子是在GDT选择；选择子的`Index=4`即`100`选择GDT中的第4个描述符；
> 
> STEP2: `OFFSET = 0x12345678` 若此时GDT第四个描述符中描述的段基址（Base）为`0x11111111`，则`线性(虚拟)地址 = 0x11111111 + 0x12345678 = 0x23456789`



## 分页寻址(虚拟内存转化为物理内存)
### 目的
> 将**分段寻址**找到的**线性(虚拟)内存地址** 转化为实际的物理地址。(PS：**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此每个应用仅能访问到自己的数据**。)
...
> **在硬件层面上提供 代码段、数据段、堆栈段 的分离，便于程序员分离(分段)管理这些数据，并在硬件层面控制每个段的读写权限，大大减少其工作量，简化其工作**。使用久了高级语言的程序员或许会觉得，分离这些东西有什么用，自己分离不就行了，但是让我们来想一想，如果CPU在硬件层面不提供这些，我们自己要分离管理这些数据的话，要怎么做呢？CPU可不能提供 List、Map 这些数据结构。是不是还挺复杂的，**即使有解决方案，性能上肯定也比不过使用 段寄存器 来直接进行分段管理**。
...
> - 内存分配以段为单位，物理内存中保存用户虚拟段到物理实际段之间的映射关系。—— 缺点：段的空间过大，以段为单位进行内存分配，容易导致申请了一大段内存，但是当前却只使用了一小部分，从而使得内存利用率较低。
> - 内存分配以每个地址为单位，物理内存中保存用户虚拟内存到物理实际内存之间的映射关系。—— 缺点：虚拟内存与实际内存一一对应，这样虚拟内存到实际内存的映射表的大小将会和物理内存本身一样大，也就是说所有的内存都会用来存放这张映射表，因此也无法实现。

通过对上面两种情况进行分析，我们会发现：

> - **以段为单位进行内存分配，容易导致申请了一大段内存，但是当前却只使用了一小部分，内存利用率太低（分页，页太大也有这个问题）**。
...
Linux 0.11中，在setup.s文件里实现了从实模式到保护模式的切换。

setup开头部分读取并保存一部分的硬件参数，初始化完硬件参数后（107 行）之后，首先使用cli指令屏蔽了所有可屏蔽的中断，防止切换过程被键盘中断之类的打断导致错误，接着从111行到127行将system模块（在bootsect.s中将system模块读到了0x10000的位置，其长度不会超过512KB（0x80000））移动到0x00000处，即：将0x10000到0x8ffff的内存块移动到0x00000到0x7ffff的位置。
...
```
GDTR中，低16位为表长度，高32位为GDT表基地址。由于INTEL使用的小端存储（little endian），因此最先出现的字0x800被存储在低16位，0x800即2048，说明该GDT最大长度为2048，后面的两个字512+gdt、0x9就落在GDTR的高32位上，0x9在前，512+gdt（注意这里的512是十进制数，即0x200）在后，于是基地址就是拼接的结果，即0x9 << 16 + 0x200 + gdt，即0x90200+gdt，在bootsect中，setup模块就被移动到了0x90200处，而gdt就是GDT在本程序段中的偏移地址，最终获得的就是GDT表的物理地址。
...
```
上面讲解过前两行，其实就是打开了PE位，第三行跳转到了段8的0偏移位置，但是注意，现在PE位已经被置为1，CPU已经进入保护模式，CPU将按照保护模式下的段寻址方式去寻找这个内存地址。

当前在CS中存储的，并不是段的地址，而是段选择子，那么0x8将段选择符的RPL和TI字段都置为0，而描述符索引就是0x1（这块就是GDT表的第一个索引，也就是代码段）。
...
## linux 0.1.1 如何在用户空间与内核空间传递数据
### 调用`/include/asm/memory.h`里的`memcpy()`方法
```
/*
...
```
    ...
    # 赋值memcpy() 中 src(SI) 的地址
    mov #100,%si    
    
    # 赋值memcpy() 中 dest(DI) 的地址
    mov #100,%di    
