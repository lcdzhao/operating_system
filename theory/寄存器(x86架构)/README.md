...
实际上在CPU执行命令的过程中，有很多数据在很长时间中都是不会进行变化的，且会频繁被CPU使用，如果这一部分数据都去进行访存的话，那么会耗费的时间将会很长，但是如果将这些数据存放在寄存器中，那么只需要在第一次加载数据到寄存器时才需要进行访存，而其他时间都可以直接从寄存器中读取数据(十分快速)。故：寄存器成为了CPU的一个重要组件。
# 寄存器的硬件电路
