...
```
上面讲解过前两行，其实就是打开了PE位，第三行跳转到了段8的0偏移位置，但是注意，现在PE位已经被置为1，CPU已经进入保护模式，CPU讲按照段模式去寻找这个内存地址。

此时在CS中存储的，并不是段的地址，而是段选择子，那么0x8将段选择符的RPL和TI字段都置为0，而描述符索引就是0x1（这块就是GDT表的第一个索引，也就是代码段）。
