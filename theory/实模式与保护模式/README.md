...
如果段选择子的 `TI = 1`，则寻址过程对应绿色那条线，其中`LDTR`为当前应用的LDT在GDT中的段选择子，因此:
> 先通过`LDTR`从`GDT`中获取到`LDT`的段描述符，其中的段基址为LDT的开始位置。再通过下面的公式获取到段描述符的物理地址：
> `段描述符物理地址 = LDT开始地址 + (Index * 段描述符大小))` —— **对应上图中的绿色的步骤1**
