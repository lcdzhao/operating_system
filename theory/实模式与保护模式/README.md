...
#### 段选择符
32位操作系统不再直接使用`段基址<<4 + 段内偏移`的方式来进行地址访问，而是使用`段选择符(cs、ds、ss、es等段寄存器) 与 段内偏移(ip、di、si、bp、sp等寄存器)`，先使用`段选择符`定位到`段描述符`，然后从`段描述符`中获取段基址，再通过`段基址 + 段内偏移`的方式来进行虚拟内存的地址访问。
