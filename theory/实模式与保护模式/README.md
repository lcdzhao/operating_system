...
32位操作系统不再直接使用`段基址<<4 + 段内偏移`的方式来进行地址访问，而是先使用`段选择符(cs、ds、ss、es等段寄存器)`定位到`段描述符`(定位过程：`段描述符物理地址 = LDTR(本地段描述符表起始位置)/GDTR(全局段描述符起始位置) + (Index * 段描述符大小))`，然后从`段描述符`中获取段基址，再通过`段基址 + 段内偏移(ip、di、si、bp、sp等寄存器)`的方式来进行虚拟内存的地址访问。(其中虚拟内存还需要进行进一步的转换才能变成物理内存，以后单独的章节进行讲解)
##### 为什么不直接使用段基址，而是使用段选择符
如果直接使用段基址，那么一个应用中的段的个数已经被CPU中的段寄存器个数所决定，其无法再进行下一步扩展，**大大限制了操作系统与应用程序的灵活性**。

而使用段选择符，则用户可以在LDT中任意扩展自己的段，操作系统可以GDT中任意扩展段(如：在GDT中保存了所有的LDT)，**大大提高了操作系统与应用程序的灵活性**。
