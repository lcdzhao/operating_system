...
32位操作系统不再直接使用`段基址<<4 + 段内偏移`的方式来进行地址访问，而是先使用`段选择符(cs、ds、ss、es等段寄存器)`定位到`段描述符`(定位过程：`段描述符物理地址 = LDTR(本地段描述符表起始位置)/GDTR(全局段描述符起始位置) + (Index * 段描述符大小))`，然后从`段描述符`中获取段基址，再通过`段基址 + 段内偏移(ip、di、si、bp、sp等寄存器)`的方式来进行虚拟内存的地址访问。
