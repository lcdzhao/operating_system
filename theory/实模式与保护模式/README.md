...
- 分页寻址：将**分段寻址**找到的**线性(虚拟)内存地址** 转化为实际的物理地址。(PS：**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此应用之间的数据被隔离了起来**。)
...
**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此应用之间的数据被隔离了起来**。
...
## 分页寻址(虚拟内存转化为物理内存)
### 目的
> 将**分段寻址**找到的**线性(虚拟)内存地址** 转化为实际的物理地址。(PS：**只要操作系统和底层硬件保证：在任何一个时刻，每个程序的虚拟地址都不会映射到相同的物理内存。这样就可以保证多个程序之间的数据隔离性。因为每个应用程序只能访问到自己的虚拟内存的地址，而不同应用的虚拟内存地址同一时刻不会映射到同一个物理内存地址，因此应用之间的数据被隔离了起来**。)
