...
Linux 0.11中，在setup.s文件里实现了从实模式到保护模式的切换。setup开始部分只是在初始化一些硬件参数。


初始化完硬件参数后（107 行）之后，首先使用cli指令屏蔽了所有可屏蔽的中断，防止切换过程被键盘中断之类的打断导致错误，接着从111行到127行将system模块（在bootsect.s中将system模块读到了0x10000的位置，其长度不会超过512KB（0x80000））移动到0x00000处，即：将0x10000到0x8ffff的内存块移动到0x00000到0x7ffff的位置。
